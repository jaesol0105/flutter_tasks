...
4. HomePageViewModel에서는 이미 state로 todolist를 다루기 때문에, detail_page에서는 VM을 통해 상태를 다루지 못했다.
5. datail_page의 VM을 따로 만들어야하나 고민하다가, 상태가 여러개일 경우 별도에 상태 클래스로 다루면 된다는 조언을 들었다. 
6. datail_page의 상태를 다룰 상태 클래스를 만들어 볼까 하다가, 헤메다가 시간상 만들지못했다.
7. 아직은 조금 복잡하게 느껴져서 시간을 두고 천천히 시도해봐야겠다.
