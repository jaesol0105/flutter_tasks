...
# 할 일을 미루지 말자.
　공부용으로 만든 반응형 UI가 적용된 Flutter TODO 앱
...
- 상태관리 : Riverpod Generator + Flutter Hooks
- Lib : cloud_firestore / geolocator / http / go_router
- API : Open-Meteo API
...
- <strong>이슈:</strong><br>무한 스크롤에 쓰로틀링을 적용해야할까?
- <strong>현재 상황:</strong><br>isLoadingMore: true를 통해 중복요청 방지 구현했음 (상태 플래그 기반)
- <strong>차이점:</strong><br>쓰로틀링 (Throttling)은 시간 간격 기반 제어
- <strong>결론:</strong><br>플래그 기반으로 호출은 한번만 되는걸 보장하기 때문에, 굳이 쓰로틀링 호출할 필요없다고 판단
